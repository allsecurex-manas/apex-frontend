...
function NetworkSecurity() {
  ...
  const networkSecurityData = getModuleData("networkSecurity");
    ...
    return (
      <div className="p-6">
        <div className="bg-white rounded-xl p-8 text-center shadow-lg">
          <div className="mb-6">
            <div className="w-16 h-16 mx-auto bg-pink-100 rounded-full flex items-center justify-center animate-pulse">
              <span className="text-3xl animate-bounce">🌐</span>
            </div>
          </div>
          <h3 className="text-2xl font-semibold text-pink-600 mb-4">Analyzing Network Security</h3>
          <p className="text-gray-600 mb-4">Please wait while we scan your network security...</p>
          <div className="w-full max-w-xs mx-auto h-2 bg-pink-100 rounded-full overflow-hidden">
  ...
  if (!scanResult || !networkSecurityData) {
    return (
      <div className="p-6">
        <div className="bg-white rounded-xl p-8 text-center shadow-lg transform transition-all hover:scale-[1.01]">
          ...
          <h3 className="text-2xl font-semibold text-pink-600 mb-4">No Network Security Data</h3>
          <p className="text-gray-600 mb-8">Start a new scan to analyze your network security posture</p>
          <button
            onClick={handleStartScan}
            className="bg-pink-600 hover:bg-pink-700 text-white px-6 py-3 rounded-lg font-semibold transition-all transform hover:scale-105 shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2"
          >
            Start Security Scan
          </button>
        </div>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3 mb-2">
              ...
              Network Security
              {hasModuleErrors("networkSecurity") && (
            ...
            <p className="text-gray-600">
              Analyzing network security for {lastScan?.domain}
              {lastScan?.time && (
          ...
          <button
            onClick={handleStartScan}
            className="inline-flex items-center px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-lg transition-colors duration-150 shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 group"
          >
      ...
      <div className="grid grid-cols-1 gap-6">
        {Object.entries(networkSecurityData).map(([domain, data]) => {
                  ...
                  <div key={severity} className="mb-6 last:mb-0">
                    ...
                    <div className="space-y-4">
                      {findings.map((finding, index) => (
                        <div 
                          key={index}
                          className="border rounded-lg p-4 hover:border-pink-200 transition-colors duration-200"
                        >
